...
function updateEngineInfo(engine) {
    ...
    const engineInfo = {
        'ocr_easyocr': {
            title: 'EasyOCR (RECOMMENDED - DEFAULT)',
            text: 'Ready-to-use OCR with 80+ language support. Excellent accuracy with no installation required. Works out of the box. Processing time: 2-4 seconds.'
        },
        'ocr_tesseract': {
            title: 'Tesseract OCR (Advanced Multi-Mode)',
            text: 'Aggressive Tesseract with 3 preprocessing methods and 4 PSM modes. Tries 12 combinations to find best result. Processing time: 3-8 seconds.'
        },
        'ocr_paddle': {
            title: 'PaddleOCR',
...
async function loadSettings() {
    try {
        ...
        // Set the saved model
        if (model === 'ocr_easyocr' && engineEasyOCR) engineEasyOCR.checked = true;
        else if (model === 'ocr_tesseract' && engineTesseract) engineTesseract.checked = true;
        ...
        else if (engineEasyOCR) engineEasyOCR.checked = true; // Default
