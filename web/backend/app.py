...
def extract_receipt_stream():
 ...
 try:
  ...
  sys.path.insert(0,os.path.join(os.path.dirname(__file__),'..','..'))
  from shared.utils.progress import ProgressTracker,ProcessingStage,register_tracker,unregister_tracker
  ...
  if not allowed_file(file.filename):
   yield 'data: '+json.dumps({'error':f'File type not allowed. Allowed types: {", ".join(ALLOWED_EXTENSIONS)}'})+'\n\n'
   return
  ...
  def generate():
   ...
   try:
    ...
    # Load model
    tracker.update(25,ProcessingStage.LOADING_MODEL,f'Loading model: {model_id or "default"}...')
    yield tracker.get_events()[-1].to_sse()
    ...
    if temp_path:safe_delete_temp_file(temp_path)
    unregister_tracker(operation_id)
