...
def check_usage_limits(f: Callable) -> Callable:
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            # Allow the request through on error to avoid blocking users
...
def increment_storage_usage(bytes_used: int) -> bool:
    ...
    try:
        ...
        user_id = getattr(g, 'user_id', None)
        if not user_id:
            logger.warning("increment_storage_usage: No user_id in context")
            return False
        
        with get_db_context() as db:
            user = db.query(User).filter(User.id == user_id).first()
            if not user:
                logger.warning(f"increment_storage_usage: User {user_id} not found")
                return False
            ...
            if user.storage_used_bytes + bytes_used > storage_limit_bytes:
                logger.warning(f"Storage limit would be exceeded: {user.storage_used_bytes + bytes_used} > {storage_limit_bytes}")
                return False
        ...
        logger.error(f"Error incrementing storage usage: {e}")
        return False
...
def decrement_storage_usage(bytes_freed: int) -> bool:
    ...
    try:
        ...
        user_id = getattr(g, 'user_id', None)
        if not user_id:
            logger.warning("decrement_storage_usage: No user_id in context")
            return False
        
        with get_db_context() as db:
            user = db.query(User).filter(User.id == user_id).first()
            if not user:
                logger.warning(f"decrement_storage_usage: User {user_id} not found")
                return False
        ...
        logger.error(f"Error decrementing storage usage: {e}")
        return False
